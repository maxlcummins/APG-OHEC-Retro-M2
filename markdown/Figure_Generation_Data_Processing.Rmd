...
fig <- plot_ly(
  ...
  layout(
    # example: scale base font size up by 50%
    font = list(size = 24),  # affects legend, hover, titles
    sunburstcolorway = ~Source_col
  ) %>%
  style(
    insidetextfont = list(size = 18),   # sector labels inside
    outsidetextfont = list(size = 18)   # sector labels outside
  )
...
if (!exists("clusters")) {
    if (file.exists("delims/plasmid_cluster_data.txt.gz")) {
        clusters <- vroom("delims/plasmid_cluster_data.txt.gz", delim = "\t")
    } else {
        source("scripts/plasmid_data_aggregate.R")
        clusters <- vroom("delims/plasmid_cluster_data.txt", delim = "\t")
    }
}
...
```
## Supplementary ARG Cluster Carriage Table
...
plasmid_abritamr_long <- plasmid_abritamr %>%
  rename(sample_id = ABRITAMR.sample_id) %>% 
  pivot_longer(
    cols = starts_with("ABRITAMR.") & !matches("ABRITAMR.sample_id"), # exclude sample_id
    names_to = "class",
    values_to = "genes"
  ) %>%
  filter(!is.na(genes) & genes != "") %>%
  mutate(
    class = str_remove(class, "^ABRITAMR\\."),   # clean the class name
    genes = str_split(genes, ",\\s*")            # split by comma
  ) %>%
  unnest(genes) %>%
  mutate(genes = str_trim(genes)) %>%
  mutate(genes = str_replace(genes, '\\*','')) # Remove asterisks which indicate partial matches

plasmid_abritamr_long <- plasmid_abritamr_long %>%
  mutate(
    genes = if_else(
      str_detect(class, "Quinolone|(?<!not_)ESBL|(?<!or_)Carbapenemase|Colistin|Macrolide|Lincosamide"),
      paste0("^", genes),
      genes
    )
  )
...
plasmid_abritamr_wide <- plasmid_abritamr_long %>%
  select(sample_id, genes) %>%
  unique() %>%
  mutate(present = 1) %>% pivot_wider(names_from = genes, values_from = present, values_fill = 0)

plasmid_abritamr_wide <- clusters %>% select(sample_id, primary_cluster_id) %>% full_join(plasmid_abritamr_wide)

arg_cluster_df <- plasmid_abritamr_wide %>%
  filter(primary_cluster_id %in% tmp_df$primary_cluster_id) %>%
    select(
    ...
    group_by(primary_cluster_id, gene) %>%
    summarise(
        n_positive = sum(presence, na.rm = TRUE),
        n_total = n(),
        prop_positive = n_positive / n_total
    )

arg_cluster_df <- arg_cluster_df %>%
  mutate(
    primary_cluster_id = factor(
      primary_cluster_id,
      levels = rev(tmp_df$primary_cluster_id)
    )
  )

ggplot(arg_cluster_df, aes(x = gene, y = primary_cluster_id, fill = prop_positive)) +
  geom_tile(color = "grey90") +
  geom_text(
    aes(
      label = ifelse(is.na(prop_positive) | prop_positive == 0, "", percent(prop_positive, accuracy = 1)),
      color = ifelse(prop_positive > 0.5, "white", "black")
    ),
    size = 3,
    angle = 90,            # rotate text vertically
    vjust = 0.5,           # center vertically in the tile
    hjust = 0.5            # center horizontally
  ) +
  scale_color_identity(guide = "none") +
  scale_fill_gradientn(
    colours = c("white", "#2b8cbe", "#de2d26"),
  ...
  labs(
    ...
    y = "Primary Cluster ID",
    title = "ARG carriage by Cluster (Most Frequently Detected Reference Plasmids)"
  ) +
...
```{r}
plasmid_vfdb_long <- clusters %>%
  filter(primary_cluster_id %in% tmp_df$primary_cluster_id) %>% 
  select(sample_id, primary_cluster_id, matches("ABRICATE..vfdb"), matches("ABRICATE..EC_custom")) %>%
  select(
      where(~ !is.numeric(.) || sum(., na.rm = TRUE) != 0)
    ) %>%
  pivot_longer(
    cols = starts_with("ABRICATE."),
    names_to = "genes",
    values_to = "present"
  ) %>%
  mutate(
    genes = str_remove(genes, "^ABRICATE\\..")
  ) %>%
  mutate(
    genes = str_replace(genes, "EC_custom_sitA_.*", "EC_custom_sitA")
  ) %>%
    group_by(primary_cluster_id, genes) %>%
    summarise(
        n_positive = sum(present, na.rm = TRUE),
        n_total = n(),
        prop_positive = n_positive / n_total
    )

plasmid_vfdb_long <- plasmid_vfdb_long %>%
  mutate(
    primary_cluster_id = factor(
      primary_cluster_id,
      levels = rev(tmp_df$primary_cluster_id)
...
unwanted_EC_custom <- c(
  "EC_custom_intI1_HQ730118.1",
  "EC_custom_F36-arc-F31",
  "EC_custom_IS1-arc-IS1",
  "EC_custom_merA_CP022086.1",
  "EC_custom_iucD_AY545598.5",
  "EC_custom_iutA_KU578032.1",
  "EC_custom_ompT_chromosomal_O157-H7_NC_002695",
  "EC_custom_senB_pUTI89_NC_007941",
  "EC_custom_terA_CP019559.1"
)

plasmid_vfdb_long <- plasmid_vfdb_long %>% filter(genes %nin% unwanted_EC_custom)

plasmid_vfdb_long <- plasmid_vfdb_long %>% mutate(genes = str_replace(genes, "EC_custom_", "^")) %>%
  mutate(genes = str_replace(genes, "vfdb_", "")) %>%
  mutate(genes = str_replace(genes, "_.*", ""))

ggplot(plasmid_vfdb_long, aes(x = genes, y = primary_cluster_id, fill = prop_positive)) +
  geom_tile(color = "grey90") +
  geom_text(
    aes(
      label = ifelse(is.na(prop_positive) | prop_positive == 0, "", percent(prop_positive, accuracy = 1)),
  ...
  labs(
    x = "Virulence Associated Gene (VAG)",
    y = "Primary Cluster ID",
    title = "VAG carriage by Cluster (Most Frequently Detected Reference Plasmids)"
  ) +
  scale_x_discrete(labels = function(x) stringr::str_wrap(x, width = 15))
```
...
```{r}
size_stats <- mobtyper_df %>%
  summarise(
    n = n(),
    min_bp = min(size, na.rm = TRUE),
    q1_bp  = quantile(size, 0.25, na.rm = TRUE, type = 7),
    med_bp = median(size, na.rm = TRUE),
    q3_bp  = quantile(size, 0.75, na.rm = TRUE, type = 7),
    max_bp = max(size, na.rm = TRUE)
  ) %>%
  mutate(
    min_kb = min_bp / 1000,
    q1_kb  = q1_bp  / 1000,
  ...
  select(n, min_kb, q1_kb, med_kb, q3_kb, max_kb)

size_stats_text <- size_stats %>%
  mutate(across(-n, ~ round(.x, 1)))  # 0.1 kb precision

methods_sentence <- size_stats_text %>%
  transmute(
    sentence = paste0(
      "Estimated sizes of plasmid constructs produced by MOBsuite  (n = ",
      n, "; median = ", med_kb, " kb; IQR = ", q1_kb, "–", q3_kb,
      " kb; range = ", min_kb, "–", max_kb, " kb)."
    )
  ) %>%
  pull(sentence)
